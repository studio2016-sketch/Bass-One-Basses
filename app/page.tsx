...
export default function Home() {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} /><main>
    ...
    <section className="section collection" id="collection"><div className="section-intro"><p className="eyebrow">Find your One</p><h2>Three expressions.<br/><em>One uncompromising standard.</em></h2><p>Every Bass One begins with a player&apos;s body and voice in mind. Choose the path that meets you where you are.</p></div><div className="model-grid">{models.map((model) => <article className="model" key={model.name}><div className="model-image"><Image src={model.image} alt={model.name} fill sizes="(max-width: 800px) 100vw, 33vw" /></div><p className="tier">{model.series}</p><h3>{model.name}</h3><p>{model.copy}</p><div className="model-bottom"><span>From {model.from}</span><a href={salesEmail(`I want to purchase an ${model.name}`)}>Inquire <b>→</b></a></div></article>)}</div></section>
